...
class ComputeMvc:
    ...
    def __init__(self,
        ...
        if self.test_w_connection is True:
            if stream_mode == 'pytrigno':
                print('Connexion to delsys trigno...')
                self.trigno_dev = pytrigno.TrignoEMG(channel_range=self.range_muscles,
                                                samples_per_read=self.sample,
    ...
    def _process_MVC(self, data, save_final_data=False, save_tmp=False, return_list=False):
        ...
        if save_tmp:
            if os.path.isfile(file_name):
                ...
                data_to_save = {f"{self.try_name}_processed": mvc_processed, "{self.try_name}_raw": data}
    ...
    def run(self, device_name=None, show_data=False, return_dev=False):
        ...
        while True:
            ...
            input(f"Ready to start trial: {self.try_name}, with muscles :{self.muscle_names}. "
            ...
            while True:
                ...
                try:
                    if nb_frame == 0:
                        print("Trial is running please press 'Ctrl+C' when trial is terminated "
            ...
            while plot_comm != 'n':
                ...
                if plot_comm == 'y':
                    plot = input(f"Press 'pr' to plot your raw trial,"
            ...
            task = input("Press 'c' to do an other MVC trial,"
            ...
            if task == 'c':
                ...
                if save != 's':
                    save = input("Data will not be saved. "
                                 "If you want to save press 's', if not, press enter.\n")
